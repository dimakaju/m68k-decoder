...
{
  public class AbsLong : ReferenceOperand
  {
    internal AbsLong(BinaryReader reader, IReadOnlyDictionary<long, AddressLabel> labels)
      : base(reader)
    {
    ...
    internal AbsLong(int address)
      => Address = address;

    public int Address { get; }

    public override OperandInfo Type => OperandInfo.AbsLong;

    internal static bool Match(int eamode, int earegister)
      => eamode == 0b111 && earegister == 0b001;
    ...
    internal override string Render(RenderingSettings settings)
    {
      ...
      result = Formatting.Address32(Address, settings);
