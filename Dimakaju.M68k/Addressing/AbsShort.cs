...
{
  public class AbsShort : ReferenceOperand
  {
    internal AbsShort(BinaryReader reader, IReadOnlyDictionary<long, AddressLabel> labels)
      : base(reader)
    {
    ...
    internal AbsShort(int address)
      => Address = address;

    public int Address { get; }

    public override OperandInfo Type => OperandInfo.AbsShort;

    internal static bool Match(int eamode, int earegister)
      => eamode == 0b111 && earegister == 0b000;

    internal override AddressLabel GenerateLabel(long address)
    {
      return new AddressLabel(Address, $"lbl{Formatting.Hex((uint)Address)}");
    }
    ...
    internal override string Render(RenderingSettings settings)
    {
      ...
      result = Formatting.Address16(Address, settings);
