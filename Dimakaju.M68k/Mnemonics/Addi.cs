...
{
  internal class Addi : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Addi;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111000000) == 0b00000110_00_000000
      || (data & 0b1111111111000000) == 0b00000110_01_000000
      || (data & 0b1111111111000000) == 0b00000110_10_000000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(8) != 0b00000110)
        return null;
