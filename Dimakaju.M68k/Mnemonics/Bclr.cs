...
{
  internal class Bclr : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Bclr;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111000111000000) == 0b0000000110000000
      || (data & 0b1111111111000000) == 0b0000100010000000;
    ...
    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      ...
      if (constant == 0b110)
      {
        ...
        instruction.Size = instruction.Operand2 is DnDirect ? Constants.Attributes.Sizes.Long : Constants.Attributes.Sizes.Byte;
        instruction.DoNotRenderSize = true;
        return instruction.Check();
      }
      else if (register == 0b100 && constant == 0b010)
      {
