...
{
    internal class Btst : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Btst;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111000111000000) == 0b0000000100000000
      || (data & 0b1111111111000000) == 0b0000100000000000;
    ...
    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      ...
      if (constant == 0b100)
      {
        ...
        instruction.Operand2 = DecodeOperand(reader, ea, 0b101111111111);
      ...
      else if (register == 0b100 && constant == 0b000)
      {
        if (reader.Read(8) != 0b00000000)
          return null;

        instruction.Operand1 = reader.Immediate8(reader, false);
        instruction.Operand2 = DecodeOperand(reader, ea, 0b101111111111);
        instruction.Size = instruction.Operand2 is DnDirect ? Attributes.Sizes.Long : Attributes.Sizes.Byte;
        instruction.DoNotRenderSize = true;
        return instruction.Check();
