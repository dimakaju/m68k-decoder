...
{
  internal class Clr : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Clr;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111000000) == 0b01000010_00_000000
      || (data & 0b1111111111000000) == 0b01000010_01_000000
      || (data & 0b1111111111000000) == 0b01000010_10_000000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(8) != 0b01000010)
        return null;
      ...
      else if (size == 0b01)
      {
        ...
        instruction.Operand1 = DecodeOperand(reader, ea, Attributes.Sizes.Word, 0b101111111000);
        return instruction.Check(dst: false);
      }
      else if (size == 0b10)
      {
        instruction.Size = Attributes.Sizes.Long;
        instruction.Operand1 = DecodeOperand(reader, ea, Attributes.Sizes.Long, 0b101111111000);
        return instruction.Check(dst: false);
      }
