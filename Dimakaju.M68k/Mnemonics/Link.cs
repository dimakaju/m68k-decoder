...
{
  internal class Link : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Link;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111111000) == 0b0100111001010000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(13) != 0b0100111001010)
        return null;

      Instruction instruction = new Instruction(Type);
      instruction.Size = Attributes.Sizes.Word;
      instruction.DoNotRenderSize = true;
      instruction.Operand1 = new AnDirect(reader.Read(3));
      instruction.Operand2 = new ImmediateData(reader.DecodeSigned16());
