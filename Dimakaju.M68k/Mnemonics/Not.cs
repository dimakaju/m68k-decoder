...
{
  internal class Not : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Not;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111000000) == 0b01000110_00_000000
      || (data & 0b1111111111000000) == 0b01000110_01_000000
      || (data & 0b1111111111000000) == 0b01000110_10_000000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(8) != 0b01000110)
        return null;
