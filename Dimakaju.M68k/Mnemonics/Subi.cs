...
{
  internal class Subi : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Subi;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111000000) == 0b00000100_00_000000
      || (data & 0b1111111111000000) == 0b00000100_01_000000
      || (data & 0b1111111111000000) == 0b00000100_10_000000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(8) != 0b00000100)
        return null;
