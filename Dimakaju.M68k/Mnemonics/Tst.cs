...
{
  internal class Tst : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Tst;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111000000) == 0b01001010_00_000000
      || (data & 0b1111111111000000) == 0b01001010_01_000000
      || (data & 0b1111111111000000) == 0b01001010_10_000000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(8) != 0b01001010)
        return null;
      ...
      if (size == 0b00)
      {
        ...
        instruction.Operand1 = DecodeOperand(reader, ea, Attributes.Sizes.Byte, 0b111111111000);
        return instruction.Check(dst: false);
      }
      else if (size == 0b01)
      {
        instruction.Size = Attributes.Sizes.Word;
        instruction.Operand1 = DecodeOperand(reader, ea, Attributes.Sizes.Word, 0b111111111000);
        return instruction.Check(dst: false);
      }
      else if (size == 0b10)
      {
        instruction.Size = Attributes.Sizes.Long;
        instruction.Operand1 = DecodeOperand(reader, ea, Attributes.Sizes.Long, 0b111111111000);
        return instruction.Check(dst: false);
      }
