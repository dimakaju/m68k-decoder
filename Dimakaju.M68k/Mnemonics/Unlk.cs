...
{
  internal class Unlk : MnemonicDecoder
  {
    public override MnemonicInfo Type => MnemonicInfo.Unlk;

    public override bool IsRelevant(ushort data)
      => (data & 0b1111111111111000) == 0b0100111001011000;

    protected override Instruction? TryDecoding(BitStreamReader reader)
    {
      if (reader.Read(13) != 0b0100111001011)
        return null;

      Instruction instruction = new Instruction(Type);
      instruction.Size = Attributes.Sizes.Unsized;
